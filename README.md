...
| Privacy | None | **Shielded by default via Ironwood** |
...
* **Shielded withdrawals** — users withdraw to a Unified shielded address (u1...), routed through Ironwood, Gleyo sends via Nozy API with memo `Gleyo ZEC Withdrawal`
...
All transactions are shielded Ironwood spends. The memo field on every withdrawal reads `Gleyo ZEC Withdrawal` so the recipient knows exactly where funds came from.
...
- All deposits are received at Gleyo's shielded Ironwood address
...
- Withdrawals are sent as shielded Ironwood transactions via Nozy API with memo `Gleyo ZEC Withdrawal`, to any Unified shielded address the user provides
...
- **Unified addresses only** — withdrawals currently require a Unified (u1...) shielded address, routed through Ironwood. Legacy Sapling-only wallets (zs1...) aren't yet supported for receiving withdrawals; users on older wallets will need to upgrade to a Unified-address wallet.
